...
| `/api/contact` | お問い合わせフォームの内容をサーバーに送信し、メール送信等の処理を行う | Next.js Client → Vercel |
...
1. `pnpm lint`, `pnpm format`, `pnpm typecheck`, `pnpm build`, `pnpm playwright test` が **すべて成功**すること
...
すべての主要クラス／サービス層クラスの冒頭には、下記 3 点を Javadoc 形式 (/** … */) で必須記載とする。
