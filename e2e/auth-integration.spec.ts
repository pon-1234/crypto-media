...
test.describe('認証方法の統合', () => {
  ...
  test('ログインページと新規登録ページ間をナビゲートできる', async ({ page }) => {
    // ログインページから開始
    await page.goto('/login')
  ...
  test.describe('セキュリティ機能', () => {
    ...
    test('パスワードリセット時のトークン有効期限', async ({ page }) => {
      ...
      // エラーメッセージが表示される
      await expect(page.getByText('リセットトークンが無効または期限切れです')).toBeVisible()
    })
  })
  ...
  test.describe('レスポンシブデザイン', () => {
    test('モバイルビューでの認証フォーム', async ({ page }) => {
      ...
      const emailButton = page.getByRole('button', { name: 'メールアドレスでログイン' })
  ...
  test.describe('アクセシビリティ', () => {
    test('キーボードナビゲーション', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'パスワードをお忘れですか？' })).toBeFocused()
    })
