...
export async function rateLimit(
  ...
  try {
    ...
    if (requestCount >= finalConfig.max) {
      // 最も古いリクエストの時刻を取得
      const oldestRequest = await redis.zrange(key, 0, 0, { withScores: true }) as Array<{member: string, score: number}>
...
export function rateLimitMiddleware(config?: Partial<RateLimitConfig>) {
  return async (request: NextRequest) => {
    ...
    response.headers.set('X-RateLimit-Limit', String(config?.max || DEFAULT_CONFIG.max))
