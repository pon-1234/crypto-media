...
describe('generatePageMetadata', () => {
  ...
  it('should generate OG image URL with default params', () => {
    ...
    expect((ogImage as { url?: string })?.url).toContain(
      'description=Test+Description'
    )
  ...
  it('should handle noindex flag', () => {
    ...
    expect(
      typeof robots === 'object' &&
        robots !== null &&
        'googleBot' in robots &&
        typeof robots.googleBot === 'object' &&
        robots.googleBot !== null &&
        'index' in robots.googleBot &&
  ...
  it('should use default base URL when env var is not set', () => {
    ...
    expect((ogImage2 as { url?: string })?.url).toContain(
      'https://crypto-media.jp/api/og?'
    )
  })
