...
describe('date utils', () => {
  describe('formatDate', () => {
    ...
    it('異なる時刻でも同じ日付として表示する', () => {
      ...
      expect(result2).toBe('2025年6月16日') // UTCからJSTへの変換で日付が変わる
    })
  ...
  describe('formatDateTime', () => {
    it('ISO 8601形式の日時を日本語形式に変換する', () => {
      ...
      expect(result).toBe('2025年1月1日 21:30') // UTC+9
    })

    it('午前0時の場合も正しく表示する', () => {
      const result = formatDateTime('2025-01-01T00:00:00.000Z')
      expect(result).toBe('2025年1月1日 09:00') // UTC+9
    })
    ...
    it('秒は表示されない', () => {
      ...
      expect(result).toBe('2025年1月1日 21:30') // 秒は省略される
    })
  })
})
