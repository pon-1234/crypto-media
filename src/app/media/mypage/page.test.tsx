...
vi.mock('next/link', () => ({
  default: vi.fn(({ children, href }: LinkProps) => (
...
describe('MyPage', () => {
  ...
  describe('無料会員の場合', () => {
    ...
    it('無料会員ステータスを表示する', async () => {
      ...
      expect(
        screen.getByText('無料記事のみお読みいただけます')
      ).toBeInTheDocument()
      // 日付は「更新」と結合されて表示される
    ...
    it('会員情報メニューがハイライトされる', async () => {
      ...
      const membershipMenuItem = screen
        .getByText('会員情報・お支払い')
        .closest('a')
  ...
  describe('有料会員の場合', () => {
    ...
    it('有料会員ステータスを表示する', async () => {
      ...
      expect(
        screen.getByText('すべての有料記事をお読みいただけます')
      ).toBeInTheDocument()
    })
  ...
  describe('メニューナビゲーション', () => {
    ...
    it('すべてのメニューに正しいリンクが設定されている', async () => {
      ...
      expect(
        container.querySelector('a[href="/media/mypage/membership"]')
      ).toBeInTheDocument()
      expect(
        container.querySelector('a[href="/media/mypage/settings"]')
      ).toBeInTheDocument()
      expect(
        container.querySelector('a[href="/media/mypage/support"]')
      ).toBeInTheDocument()
    })
