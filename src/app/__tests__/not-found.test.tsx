...
describe('NotFound', () => {
  it('404エラーメッセージが表示される', () => {
    ...
    expect(screen.getByText('お探しのページは、移動または削除された可能性があります。')).toBeInTheDocument();
  });

  it('コーポレートサイトへのリンクが表示される', () => {
    render(<NotFound />);
    
    const corporateLink = screen.getByRole('link', { name: 'コーポレートサイトへ' });
  ...
  it('適切なスタイルクラスが適用されている', () => {
    ...
    expect(wrapper).toHaveClass('min-h-screen', 'flex', 'items-center', 'justify-center', 'bg-gray-50');
  });
});