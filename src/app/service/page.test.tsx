...
describe('ServicePage', () => {
  ...
  it('すべてのサービスセクションが表示される', () => {
    ...
    // メインセクション
    expect(
      screen.getByRole('heading', { name: '提供サービス' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('heading', { name: 'サービスの特徴' })
    ).toBeInTheDocument()
    expect(
    ...
    expect(
      screen.getByRole('heading', {
        name: '1. 暗号資産メディア「Crypto Media」の運営',
      })
    ).toBeInTheDocument()
    expect(
    ...
    expect(
      screen.getByRole('heading', { name: '3. 有料会員サービス' })
    ).toBeInTheDocument()
    expect(
      screen.getByRole('heading', { name: '4. 教育・啓蒙活動' })
    ).toBeInTheDocument()
  })
