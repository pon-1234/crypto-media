...
describe('Footer', () => {
  ...
  it('should render all media links', () => {
    ...
    const mediaNewsLink = newsLinks.find(link => link.getAttribute('href') === '/media/category/news')
  ...
  it('should render all legal links', () => {
    ...
    expect(screen.getByRole('link', { name: '特定商取引法に基づく表記' })).toHaveAttribute('href', '/commercial')
  })

  it('should render contact section with link', () => {
    render(<Footer />)
    
    const contactHeading = screen.getAllByText('お問い合わせ').find(el => el.tagName === 'H3')
    expect(contactHeading).toBeInTheDocument()
    expect(screen.getByText('ご質問やご意見がございましたら、お気軽にお問い合わせください。')).toBeInTheDocument()
  ...
  it('should render copyright with current year', () => {
    ...
    expect(screen.getByText(`© ${currentYear} 株式会社[会社名]. All rights reserved.`)).toBeInTheDocument()
  })
