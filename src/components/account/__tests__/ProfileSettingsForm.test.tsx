...
describe('ProfileSettingsForm', () => {
  ...
  it('名前を変更して送信できる', async () => {
    ...
    const updateProfile = await import('@/app/actions/account').then(m => m.updateProfile)
    ...
    await waitFor(() => {
      expect(updateProfile).toHaveBeenCalledWith('test-user-id', { name: 'New Name' })
    })
  })
  ...
  it.skip('更新中はフォームが無効になる', async () => {
    ...
    vi.mocked(updateProfile).mockReturnValue(promise as Promise<{ success?: boolean; error?: string }>)
  ...
  it('エラーが発生した場合はエラーメッセージを表示する', async () => {
    ...
    vi.mocked(updateProfile).mockResolvedValue({ error: 'プロフィールの更新に失敗しました' })
